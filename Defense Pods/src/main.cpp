...
// OpenAL Defines
#define NUM_BUFFERS 4 // Se agrego un nuevo buffer, 3 -> 4
...
int ch;
ALboolean loop;
std::vector<bool> sourcesPlay = {true, true, true, true}; // Se agrego un nuevo elemento al vector
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	buffer[3] = alutCreateBufferFromFile("../sounds/car_acel.wav"); // Se agrego esta linea
...
void applicationLoop() {
	...
	// Posicion light, apuntador a la camara, distancia maxima de alcance, plano cercano y el FOV (Fiel of view)
	...
	while (psi) {
		...
		// Agregando la posicion de la fuente de sonido
		source3Pos[0] = modelMatrixLambo[3].x;
		source3Pos[1] = modelMatrixLambo[3].y;
		source3Pos[3] = modelMatrixLambo[3].z;
		alSourcefv(source[3], AL_POSITION, source3Pos);
